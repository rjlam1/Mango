...
const Banner = () => {
  ...
  return (
    <div className="relative w-full h-[90vh] max-h-[600px] overflow-hidden shadow-2xl">
  <AnimatePresence>
    <motion.div
      key={current}
      initial={{ opacity: 0 }}
  ...
  <div className="absolute z-10 flex items-center justify-start h-full px-6 md:px-16">
    ...
    >
      <h1 className="text-3xl font-extrabold leading-tight md:text-5xl drop-shadow-lg">
        {slides[current].title}
      </h1>
      <p className="text-base font-light md:text-lg drop-shadow-md">
        {slides[current].description}
      </p>
      <button className="px-6 py-2 mt-2 text-sm font-semibold transition-all duration-300 bg-green-500 rounded-full shadow-lg hover:bg-green-600">
        Learn More
      </button>
    </motion.div>
  </div>
